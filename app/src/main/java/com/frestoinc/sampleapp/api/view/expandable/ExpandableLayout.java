...
public class ExpandableLayout extends FrameLayout {
    ...
    /**
     * Generates a new Expandable layout.
     *
    ...
    public ExpandableLayout(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            parallax = a.getFloat(R.styleable.ExpandableLayout_parallax, 1);
            a.recycle();

            state =
                    expansion == 0
                            ? State.getStatusInt(State.COLLAPSED)
                            : State.getStatusInt(State.EXPANDED);
    ...
    public boolean isExpanded() {
        return state == State.getStatusInt(State.EXPANDING)
                || state == State.getStatusInt(State.EXPANDED);
    }
    ...
    public void setExpansion(float expansion) {
        ...
        if (expansion == 0) {
            state = State.getStatusInt(State.COLLAPSED);
        } else if (expansion == 1) {
            state = State.getStatusInt(State.EXPANDED);
        } else if (delta < 0) {
            state = State.getStatusInt(State.COLLAPSING);
        } else if (delta > 0) {
            state = State.getStatusInt(State.EXPANDING);
        }

        setVisibility(state == State.getStatusInt(State.COLLAPSED) ? GONE : VISIBLE);
    ...
    private void animateSize(int targetExpansion) {
        ...
        animator.addUpdateListener(
                valueAnimator -> setExpansion((float) valueAnimator.getAnimatedValue()));
    ...
    private class ExpansionListener implements Animator.AnimatorListener {
        ...
        public void onAnimationStart(Animator animation) {
            state =
                    targetExpansion == 0
                            ? State.getStatusInt(State.COLLAPSING)
                            : State.getStatusInt(State.EXPANDING);
        }
